...
app.get('/api/me', (req, res) => {
  ...
  res.json({
    user: {
      ...
      plan: (req as any).tenantPlan ?? 'basic',
    },
  });
});
...
app.get('/api/me/team', async (req, res) => {
  if (!CLOUD_ENABLED) { res.json({ members: [] }); return; }
  const supabaseTenantId = (req as any).supabaseTenantId as string | undefined;
  if (!supabaseTenantId) { res.json({ members: [] }); return; }

  const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || '';
  const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) { res.json({ members: [] }); return; }
  ...
  try {
    ...
    const memberRes = await fetch(
      `${SUPABASE_URL}/rest/v1/tenant_members?tenant_id=eq.${supabaseTenantId}&select=user_id`,
      { headers }
    );
    if (!memberRes.ok) { res.json({ members: [] }); return; }
    const memberRows = await memberRes.json() as Array<{ user_id: string }>;
    if (!memberRows.length) { res.json({ members: [] }); return; }

    // 2. Fetch each user's email from the auth admin API (parallelised)
    const userDetails = await Promise.all(
      memberRows.map(async ({ user_id }) => {
        try {
