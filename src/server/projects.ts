...
export function reorderProjects(ids: string[], dataDir: string = DATA_DIR): Project[] {
  const projects = listProjects(dataDir);
  ...
  for (const id of existingIds) {
    if (!incoming.has(id)) throw new Error(`Reorder missing id: ${id}`);
  }
...
export function setProjectLogo(id: string, filename: string, dataDir: string = DATA_DIR): Project {
  const projects = listProjects(dataDir);
  const idx = projects.findIndex(p => p.id === id);
  if (idx === -1) throw new Error(`Project not found: ${id}`);
...
export function clearProjectLogo(id: string, dataDir: string = DATA_DIR): Project {
  const projects = listProjects(dataDir);
  const idx = projects.findIndex(p => p.id === id);
  if (idx === -1) throw new Error(`Project not found: ${id}`);
...
export function getProjectLogoPath(id: string, dataDir: string = DATA_DIR): string | null {
  const project = getProject(id, dataDir);
  if (!project?.logo) return null;
  // Defensive: refuse path-traversal in stored filename.
  if (project.logo.includes('/') || project.logo.includes('..') || project.logo.includes('\\')) return null;
