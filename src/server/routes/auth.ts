...
<body>
  <div class="login-wrap">
    ...
    <div class="login-card">
      ...
      <div class="login-footer">
        <a href="https://yoinko.ai">← back to yoinko.ai</a>
      </div>
    </div>
  </div>
  ...
  <script type="module">
    ...
    // Check if we already have a session (from shared cookies)
    const { data: { session } } = await supabase.auth.getSession();
    if (session?.access_token) {
      await setTokenAndRedirect(session.access_token);
    }
...
<body>
  ...
  <script type="module">
    ...
    try {
      ...
      // The client library auto-detects and exchanges them
      ...
      // Store the token in an httpOnly cookie via our API
      const res = await fetch('/auth/set-token', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
